...
@Component({
  selector: 'app-gamblerspage',
  templateUrl: './gamblerspage.component.html',
  styleUrls: ['./gamblerspage.component.css']
})
export class GamblerspageComponent implements OnInit
{

  apostadoresData: apostadoresDTO = new apostadoresDTO; // Instancia que almacena los datos de los apostadores
  ...
  selectedEmail: string | null = null; // Almacena el email de la tabla.
  ...
  controlPuntaje: number | null = null; // Variable que controla el puntaje en controlGamblers

  constructor( private apostadoresService: apostadoresService, private sharedDataService: SharedDataService, private router: Router )
  {
  ...
  onDataTable(){
    ...
    this.apostadoresService.getApostadores(0, "empty", "empty").subscribe(res => {
  ...
  searchItem(): void{
    if (this.apostadoresData.apostadoresDTOid) 
    {
      // Buscar un elemento específico según el ID proporcionado
      this.apostadoresService.getApostadores(this.apostadoresData.apostadoresDTOid, "empty", "empty").subscribe(res => 
      {
  ...
  clearPostData(){ //TODO (Revisar esquema original)
    ...
    this.selectedName = 'Nuevo apostador';
  ...
  deleteItem(): void{ //TODO (Revisar esquema original, metodo con el nombre borrarElemento)
    const confirmacion = confirm('¿Estás seguro de eliminar a ' + this.selectedName + '?');
    ...
    if (confirmacion) {
      ...
      if (this.deleteCheck && this.idSeleccionado != null && this.selectedName != null) {
        ...
        this.apostadoresService.deleteApostadores(this.idSeleccionado, this.selectedName, "empty", 0).subscribe(
          () => {
  ...
  selectItemAndStoreNombre(item: any) {
    ...
    this.datatable[0].forEach((element: any) => {
      if (element !== item) {
        ...
        this.selectedName = item.aposNombre;
  ...
  selectItemAndStoreMail(item: any) {
    this.datatable[0].forEach((element: any) =>{
      if (element !== item) {
        element.seleccionado = false;
      } else {
        this.selectedEmail = item.apostMail;
        console.log("selectItemAndStoreMail funciona correctamente");
        if (this.selectedEmail != null){
          this.sharedDataService.setEmailSeleccionado(this.selectedEmail)
        }
      }
    }) 
  }
