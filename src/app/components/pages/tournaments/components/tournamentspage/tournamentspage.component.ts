...
@Component({
  selector: 'app-tournamentspage',
  templateUrl: './tournamentspage.component.html',
  styleUrls: ['./tournamentspage.component.css']
})
export class TournamentspageComponent implements OnInit 
{
  competenciasData: competenciasDTO = new competenciasDTO; // Instancia que almacena los datos
  ...
  constructor( private competenciasService: competenciasService, private sharedDataService: SharedDataService, private router: Router )
  {
  ...
  onDataTable(){
    // Obtener los datos de la tabla mediante el servicio
  ...
  searchItem(): void{
    if (this.competenciasData.competenciasDTOid) 
    {
      // Buscar un elemento específico según el ID proporcionado
      this.competenciasService.getCompetencia(this.competenciasData.competenciasDTOid, "empty").subscribe(res => 
      {
        console.log(res); // Imprimir el objeto res en la consola
        if ((res as any).competencia.length === 0) 
        {
  ...
  deleteItem(): void{ //TODO (Revisar esquema original, metodo con el nombre borrarElemento)
    ...
    if (confirmacion) {
      ...
      if (this.deleteCheck && this.idSeleccionado != null && this.selectedName != null) {
        // Llamada al método deletePaies() del servicio PaisesService pasando el ID y el nombre de la competencia
        this.competenciasService.deleteCompetencias(this.idSeleccionado, this.selectedName).subscribe(
          () => {
  ...
  selectItemAndStoreID(item: any) {
    ...
    this.datatable[0].forEach((element: any) => {
      if (element !== item) {
        ...
        this.idSeleccionado = item.idCompetencia;
  ...
  selectItemAndStoreNombre(item: any) {
    ...
    this.datatable[0].forEach((element: any) => {
      if (element !== item) {
        ...
        this.selectedName = item.competenciaNombre;
