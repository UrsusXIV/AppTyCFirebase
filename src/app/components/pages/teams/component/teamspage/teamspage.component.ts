...
export class TeamspageComponent implements OnInit {
  ...
  constructor( private equiposService: equiposService, private sharedDataService: SharedDataService, private router: Router )
  {
  ...
  onDataTable(){
    ...
    this.equiposService.getEquipos(0, "empty").subscribe(res => {
  ...
  searchItem(): void{
    if (this.equiposData.equiposDTOid) 
    {
      // Buscar un elemento específico según el ID proporcionado
      this.equiposService.getEquipos(this.equiposData.equiposDTOid, "empty").subscribe(res => 
      {
  ...
  deleteItem(): void{ //TODO (Revisar esquema original, metodo con el nombre borrarElemento)
    ...
    if (confirmacion) {
      ...
      if (this.deleteCheck && this.idSeleccionado != null && this.selectedName != null) {
        ...
        this.equiposService.deleteEquipos(this.idSeleccionado, this.selectedName).subscribe(
          () => {
